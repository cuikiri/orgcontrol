...
public class AtividadeResource {
    ...
    @Timed
    public ResponseEntity<List<Atividade>> getAllAtividades(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Atividades");
        Page<Atividade> page;
        if (eagerload) {
            page = atividadeService.findAllWithEagerRelationships(pageable);
        } else {
            page = atividadeService.findAll(pageable);
        }
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, String.format("/api/atividades?eagerload=%b", eagerload));
