...
export class ResponsavelService {
    ...
    protected convertDateFromClient(responsavel: IResponsavel): IResponsavel {
        const copy: IResponsavel = Object.assign({}, responsavel, {
            data: responsavel.data != null && responsavel.data.isValid() ? responsavel.data.format(DATE_FORMAT) : null
        });
        return copy;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((responsavel: IResponsavel) => {
                responsavel.data = responsavel.data != null ? moment(responsavel.data) : null;
            });
        }
        return res;
    }
